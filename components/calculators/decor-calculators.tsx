...
function MeasurementSystemField({ hint, system, onChange }: { hint: string; system: MeasurementSystem; onChange: (system: MeasurementSystem) => void }) {
  return (
    <div>
...
function ToolLayout({ calculatorSlug, children, copyDisabled, copyStatus, instructions, onCopy, onReset, result, resultTitle, summary, title }: { calculatorSlug: CalculatorSlug; children: ReactNode; copyDisabled: boolean; copyStatus: string; instructions: string; onCopy: () => void; onReset: () => void; result: ReactNode; resultTitle: string; summary: string; title: string }) {
  ...
  return (
    <section className="grid gap-5 xl:grid-cols-[minmax(0,1.1fr)_minmax(21rem,0.9fr)] xl:items-start xl:gap-8">
      <form className="rounded-2xl bg-card p-5 text-card-foreground shadow-sm sm:p-7 lg:p-8" noValidate onSubmit={(event) => event.preventDefault()}>
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">{instructions}</p>
        {children}
...
function invalidResultSummary(count: number) {
  return `Result unavailable. ${count} ${count === 1 ? "field needs" : "fields need"} attention.`;
}
...
export function FurnitureFitCalculator() {
  ...
  const summary = result ? `Furniture ${result.fitsRoom ? "fits" : "does not fit"} the room; requested clearance ${result.meetsClearance ? "is" : "is not"} available.` : invalidResultSummary(errors.length);
...
export function CurtainMeasurementCalculator() {
  ...
  const summary = result ? `Suggested rod width ${formatLength(result.rodWidthCm, system)}, ${result.panelCount} panels, finished drop ${formatLength(result.dropLengthCm, system)}.` : invalidResultSummary(errors.length);
  ...
  return <ToolLayout calculatorSlug="curtain-measurement" copyDisabled={!result} copyStatus={copyStatus} instructions="Use the outside window width, available wall space, packaged panel width, and your intended finished drop. The panel result always rounds up to a whole item." onCopy={copy} onReset={() => { setRaw(curtainDefaults); setFullness(2); setSystem("metric"); setCopyStatus("Defaults restored."); }} resultTitle="Curtain and rod estimate" summary={summary} title="Plan ready-made curtain panels" result={result ? <><div className="mt-6 rounded-lg border border-primary/20 bg-background/70 p-5"><p className="text-sm text-muted-foreground">Whole panels to buy</p><p className="mt-1 font-mono text-4xl font-semibold">{result.panelCount}</p><p className="mt-2 text-sm text-muted-foreground">Rounded up at {fullness}× fullness</p></div><dl className="mt-6 divide-y divide-border text-sm"><ResultRow label="Suggested rod width" value={formatLength(result.rodWidthCm, system)} /><ResultRow label="Combined fabric width" value={formatLength(result.totalFabricWidthCm, system)} /><ResultRow label="Finished drop" value={formatLength(result.dropLengthCm, system)} /></dl><p className="mt-6 text-xs leading-5 text-muted-foreground">Check the panel header, rings, hems, pattern repeat, shrinkage, and manufacturer instructions before ordering or cutting fabric.</p></> : <InvalidResult count={errors.length} />}>
    <div className="mt-7 grid gap-5 sm:grid-cols-2"><MeasurementSystemField hint="Changing units converts the window, rod, panel, and drop measurements as one set." system={system} onChange={changeSystem} /><div><Label htmlFor="curtain-fullness">Fullness</Label><Select className="mt-2" id="curtain-fullness" value={String(fullness)} onValueChange={(value) => setFullness(Number(value) as CurtainFullness)}><option value="1.5">1.5× light</option><option value="2">2× standard</option><option value="2.5">2.5× full</option><option value="3">3× very full</option></Select><p className="mt-2 text-xs leading-5 text-muted-foreground">The cited IKEA guide illustrates 2× and 2.5×. Match the choice to your panel header and product instructions.</p></div></div>
